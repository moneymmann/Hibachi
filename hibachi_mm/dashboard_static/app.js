...
$('events').textContent='waiting for engine events...';
async function refresh(){
  const s=await api('/api/summary');
  $('engine').textContent=JSON.stringify(s.engine||{status:'WAITING',mode:'dry-run'},null,2);
  $('account').textContent=JSON.stringify(s.account||{equity:'-'},null,2);
  $('risk').textContent=JSON.stringify(s.risk||{scenario:'WAITING'},null,2);
  $('market').textContent=JSON.stringify(s.market||{mid:'-'},null,2);
  $('modeBadge').textContent=(s.engine?.mode||'DRY RUN').toUpperCase();
  $('modeBadge').className='badge '+((s.engine?.mode||'dry-run').replace(' ','-'));
  const orders=await api('/api/open-orders');
  $('orders').innerHTML='<tr><th>orderId</th><th>side</th><th>role</th><th>price</th><th>qty</th><th>status</th></tr>'+orders.map(o=>`<tr><td>${o.order_id}</td><td>${o.side}</td><td>${o.order_role}</td><td>${o.price}</td><td>${o.qty}</td><td>${o.status}</td></tr>`).join('');
